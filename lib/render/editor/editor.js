$(document).ready(function () {
  ...
  mumuki.kids.registerStatesScaler(($state, fullMargin, preferredWidth, preferredHeight) => {
    const $table = $state.find('.active gs-board > table');
    if(!$table.length) return setTimeout(() => mumuki.kids.scaleStates($state, fullMargin));

    $table.css('transform', 'scale(1)');
    $table.css('transform', 'scale(' + Math.min(preferredWidth / $table.width(), preferredHeight / $table.height()) + ')');
  });

  mumuki.kids.registerBlocksScaler(($blocks) => {
